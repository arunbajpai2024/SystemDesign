...
public class INR500Dispenser implements DispenserChain {

    private DispenserChain dispenserChain;

    @Override
    public void dispense(Currency currency) {
        if (currency.getValue() >= 500) {
            int num = currency.getValue()/500;
            System.out.println("Dispensing " + num + " INR500 note");
            int remainder = currency.getValue()%500;
            if (remainder>0) {
